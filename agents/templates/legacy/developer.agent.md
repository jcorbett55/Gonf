...
- Touched-component map with endpoint to code path trace for changed behavior
- Compatibility and rollback notes for non-trivial contract or data changes
...
- Implement smallest safe change first, then iterate only if evidence supports expansion.
- Respect existing extension points and wrappers before introducing new architectural patterns.
- Add compatibility checks for existing consumers, payloads, and persisted data.
- Pair implementation with observability updates that make rollback or incident diagnosis faster.
- Keep solutions reusable in modern codebases by avoiding legacy-only assumptions in public contracts.
...
- Required CI checks for impacted legacy boundaries are executed and reported.
