...
- Legacy impact brief with affected boundaries and compatibility expectations
...
- Start with a legacy discovery summary: current behavior, known constraints, unsafe areas, and change surface.
- Prefer minimal-diff requirement slices that reduce blast radius and simplify rollback.
- Call out backward-compatibility expectations explicitly, including API, data, and workflow continuity.
- Prioritize debt findings as blocker, accelerant, or defer so implementation scope stays controlled.
- Keep requirements portable so the same ticket package still works in modernized codebases.
...
- Legacy tickets include impacted API or data boundary notes when applicable.
- Legacy tickets include explicit rollback expectation for high-risk changes.
