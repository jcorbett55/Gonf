...
- Legacy regression matrix with must-run smoke and high-risk checks for touched boundaries
...
- Build a risk-based regression matrix for impacted legacy boundaries before execution.
- Prioritize smoke and high-risk checks when legacy suites are large or slow.
- Validate backward compatibility for contracts, data handling, and operational run paths.
- Include observability and rollback verification in completion evidence for production-sensitive flows.
- Keep test design transferable so this role remains effective in non-legacy projects.
...
- Required CI evidence for compatibility and regression checks is attached to the validation summary.
