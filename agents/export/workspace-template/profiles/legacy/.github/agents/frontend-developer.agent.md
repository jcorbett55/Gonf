...
- Touched-surface map for screens, components, and API integration points
- Compatibility notes for legacy state assumptions and fallback behavior
...
- Favor incremental UI improvements that fit existing component and styling patterns.
- Avoid broad visual rewrites unless the ticket explicitly authorizes modernization scope.
- Validate compatibility with legacy API shapes and state assumptions before introducing new client models.
- Include regression checks for nearby screens and user journeys that share legacy dependencies.
- Keep implementation choices portable so the same role can operate effectively in newer frontend stacks.
...
- Required CI checks for impacted UI and integration boundaries are executed and reported.
